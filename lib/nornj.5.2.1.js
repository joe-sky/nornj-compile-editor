...
(function (global, factory) {
  ...
  (function (ExtensionPrefixConfig) {
    ExtensionPrefixConfig["onlyLowerCase"] = "onlyLowerCase";
    ExtensionPrefixConfig["onlyUpperCase"] = "onlyUpperCase";
    ExtensionPrefixConfig["free"] = "free";
  })(exports.ExtensionPrefixConfig || (exports.ExtensionPrefixConfig = {}));
  ...
  var extensionConfig = {
    ...
    "switch": _config(_defaultCfgExpInProps, {
      needPrefix: exports.ExtensionPrefixConfig.onlyLowerCase
    }),
  ...
  function _getProp(matchArr, innerQuotes, i, addSet) {
    ...
    var item = [matchArr[0], matchArr[1], null, true];
  ...
  function compiledParam(value, tmplRule, hasColon, onlyKey, addSet) {
    ...
    var strs = isStr ? !hasColon ? value.split(tmplRule.replaceSplit) : ['', ''] : [value],
        props = null,
  ...
  function _buildFn(content, node, fns, no, newContext, level, useStringLocal) {
    var fnStr = '';
    var useString = useStringLocal != null ? useStringLocal : fns.useString,
        main = no === 0,
  ...
  function _buildOptions(config, useStringLocal, node, fns, level, hashProps, tagName, tagProps) {
    ...
    if (node) {
      ...
      if (noConfig || config.hasName) {
        hashStr += ", name: '".concat(node.ex, "'");
      }

      if (tagName && isDirective && (noConfig || !config.noTagName)) {
        hashStr += ', tagName: ' + tagName;
        hashStr += ", setTagName: function(c) { ".concat(tagName, " = c }");
      }
  ...
  function _buildProps(obj, fns, useStringLocal, level) {
    ...
    if (isString(str0)) {
      //常规属性
      valueStr = !obj.isAll && str0 !== '' ? "'".concat(_replaceStrs(str0), "'") : '';
      each(obj.props, function (o, i) {
        ...
        if (!obj.isAll) {
          ...
          dataValueStr = prefixStr + '(' + dataValueStr + ')' + (strI !== '' ? " + '".concat(_replaceStrs(strI), "'") : '');
        } else {
    ...
    if (params || hasPropsEx) {
      ...
      if (params) {
        ...
        each(paramKeys, function (k, i) {
          ...
          var onlyKey = params[k].onlyKey;

          if (!useStringF) {
            key = fixPropName(key);
          }

          paramsStr += "  '" + key + "': " + (!onlyKey ? valueStr : !useString ? 'true' : "'".concat(key, "'")) + (i < len - 1 ? ',\n' : '');
        }, false);
        paramsStr += '\n};\n';
      }
  ...
  function _buildNode(node, parent, fns, counter, retType, level, useStringLocal, isFirst, tagName, tagProps) {
    ...
    if (node.type === 'nj_plaintext') {
      ...
      var _exC = counter._ex++,
          ...
          exVarStr = '_ex' + _exC,
          globalExStr = "".concat(GLOBAL, ".x['").concat(node.ex, "']");
      ...
      if (configE && configE.onlyGlobal) {
        ...
        fnStr += '\nvar ' + exVarStr + ';\n';
        fnStr += 'var ' + fnHVarStr + " = ".concat(CONTEXT, ".d('").concat(node.ex, "', 0, true);\n");
        fnStr += 'if (' + fnHVarStr + ') {\n';
      ...
      {
        //如果扩展标签不存在则打印警告信息
        fnStr += "".concat(GLOBAL, ".tf(_ex").concat(_exC, ", '").concat(node.ex, "', 'ex');\n");
      } //渲染
      ...
      if (!useStringF) {
        ...
        typeStr = _typeRefer ? "".concat(GLOBAL, ".en(") + _typeRefer + ", '" + _type + "')" : "'".concat(_type, "'");
      }
  ...
  function _checkStringElem(xml, tmplRule, outputH) {
    ...
    while (matchArr = pattern.exec(xml)) {
      ...
      var textAfter = matchArr[5]; //处理上一次循环中的可省略闭合标签
  ...
  function _setElem(elem, elemName, elemParams, elemArr, bySelfClose, tmplRule, outputH) {
    var ret, paramsEx;
    var fixedExTagName = fixExTagName(elemName, tmplRule);
      ...
      name = name.replace(REGEX_EX_ATTR, function (all, name, modifier, g3, arg) {
        if (arg) {
          args = arg.substr(1).split('-').map(function (item) {
            ...
            strs.forEach(function (str, i) {
              if (i == 0) {
                argStr = "name:'".concat(str, "'") + (i < strs.length - 1 ? ',' : '');
              } else {
                modifierStr += "'".concat(str, "'") + (i < strs.length - 1 ? ',' : '');
              }
            });
